...
public class TransparentButton extends Button {
    ...
    public TransparentButton(int x, int y, int width, int height, String title, IPressable pressedAction) {
     ...
     */
    public static void drawPlainCenteredString(
        @Nonnull FontRenderer fontRenderer, String text,
        int x, int y, int color
    ) {
        fontRenderer.drawString(
            text,
            (float)(x - fontRenderer.getStringWidth(text) / 2), (float)y, color
        );
    }
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        if (visible) {
            ...
            TransparentButton.drawPlainCenteredString(
                minecraft.fontRenderer, getMessage(),
